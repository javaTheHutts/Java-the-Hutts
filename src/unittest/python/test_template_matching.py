...
r = 900.0/brick.shape[1]
dim = (900,int(brick.shape[0]*r))
...
def test_match_template():
    template = TemplateMatching()
    cv2.setNumThreads(0)
    assert template.identify(direct_match) == 'idcard'
    cv2.setNumThreads(-1)

def test_wrong_dim_template():
    template = TemplateMatching()
    cv2.setNumThreads(0)
    assert template.identify(wrong_dim) == 'None'
    cv2.setNumThreads(-1)

def test_no_match_template():
    template = TemplateMatching()
    cv2.setNumThreads(0)
    assert template.identify(no_match) == 'None'
    cv2.setNumThreads(-1)

def test_student_template():
    template = TemplateMatching()
    cv2.setNumThreads(0)
    assert template.identify(student_card) == 'studentcard'
    cv2.setNumThreads(-1)

def test_to_small_template():
    template = TemplateMatching()
    cv2.setNumThreads(0)
    assert template.identify(id_book) == 'None'
    cv2.setNumThreads(-1)